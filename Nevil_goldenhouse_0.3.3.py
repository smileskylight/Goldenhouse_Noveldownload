...
def declare():
    ...
    print(' 為避免造成網站癱瘓,nl小說編號列表會預先下載成"小說編號列表.txt"檔案再做判讀')
...
def nh():
    with open('小說編號列表.txt', 'r', encoding='utf-8') as file:
        ...
        matches = re.findall(f'書籍名稱: ({bookname}.*?)書籍編號: (\\d+)', nevil_read, re.DOTALL)
...
def nva():
    ...
    for href in hrefs:
        ...
        # 抓內容
        nevil = root.find_all("div", style="font-size: 20px; line-height: 30px; word-wrap: break-word; table-layout: fixed; word-break: break-all; width: 750px; margin: 0 auto; text-indent: 2em;")
