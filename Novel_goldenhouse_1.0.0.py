...
def nsd():
    ...
    # 打開文本文件並讀取內容
    nevolname = input("請輸入小說名字 : ")
    with open(f'./{nevolname}.txt', 'r', encoding='utf-8') as file:
    ...
    for i, sentence in enumerate(sentences, start=1):
        ...
        # 下載 MP3 文件
        std_output = subprocess.run(download_cmd)
        ...
        convert_cmd = [
            ...
            f'{output_dir}/{t}.mp3',
            '-filter:a',
            'asetrate=44100*0.48,atempo=1.95',
            f'{output_dir}/{t}_atempo.mp3'
        ]

        # 轉換 MP3 文件
        std_output = subprocess.run(convert_cmd)
        if std_output.returncode == 0:
            print(f'[{t}_atempo] 轉換成功')
        else:
            print(f'[{t}_atempo] 轉換失敗')

    # 定義合併指令
    ffmpeg_cmd = [
        r'./ffmpeg/bin/ffmpeg.exe',
        '-i',
        f'concat:{"|".join([f"{output_dir}/第{i}句_atempo.mp3" for i in range(1, len(sentences) + 1)])}',
        '-c',
        'copy',
        f'{output_dir}/{nevolname}.mp3'
    ]

    # 合併 MP3 文件
    std_output = subprocess.run(ffmpeg_cmd)
    ...
    if std_output.returncode == 0:
        ...
        for i in range(1, len(sentences) + 1):
            os.remove(f'{output_dir}/第{i}句_atempo.mp3')
            os.remove(f'{output_dir}/第{i}句.mp3')
        print('已刪除已合併的單個 MP3 文件！')
    else:
        print('MP3 文件合併失敗！')
